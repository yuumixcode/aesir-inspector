...
namespace RunLab.AesirInspector
{
    ...
    public static class TypeAnalyzerStaticExtensions
    {
            ...
            out object defaultValue)
        {
            ...
            if (isStatic)
            {
                var staticValue = OdinInspectorSafeEditorUtility.GetMemberValue(propertyInfo, null);
                if (staticValue == null ||
        ...
        {
            var targetTypeName = OdinInspectorSafeEditorUtility.GetNiceName(type);

            if (useFullName)
            {
                targetTypeName = OdinInspectorSafeEditorUtility.GetNiceFullName(type);
            }
        ...
        {
            var links = OdinInspectorSafeEditorUtility.GetAttributes<ReferenceLinkURLAttribute>(type);
        ...
        {
            return OdinInspectorSafeEditorUtility.GetBaseTypes(type).Where(t => !t.IsInterface).Select(baseType =>
        ...
        public static string GetMethodNameAndParameters(this MethodBase method)
        {
            ...
            if (method.IsGenericMethod)
            {
                ...
                for (var index = 0; index < genericArguments.Length; ++index)
                {
                    ...
                    stringBuilder.Append(OdinInspectorSafeEditorUtility.GetNiceName(genericArguments[index]));
                }
            ...
            {
                stringBuilder.Append(
                    " " + OdinInspectorSafeEditorUtility.GetReturnType(method).GetReadableTypeName());
            }
        ...
        public static string GetParametersNameWithDefaultValue(this MethodBase method)
        {
            ...
            for (var length = parameterInfoArray.Length; index < length; ++index)
            {
                ...
                if (index == 0 && OdinInspectorSafeEditorUtility.IsExtensionMethod(method))
                {
