    ...
    public static class GameObjectSerializer
    {
        ...
        public static object GetGameObjectData(GameObject go)
        {
            ...
            return new
            {
                ...
                parentInstanceID = go.transform.parent?.gameObject.GetStableInstanceId() ?? 0, // 0 if no parent
                ...
                // NOTE: GetComponents<Component>() returns null entries for missing/broken script
                // references (e.g. a renamed/deleted script, a compile error, or a non-UTF-8 script
                // that failed to load). Filter them out so serialization doesn't throw NRE.
        ...
        {
            // --- Add Early Logging --- 
            // CodelyLogger.Log($"[GetComponentData] Starting for component: {c?.GetType()?.FullName ?? "null"} (ID: {c?.GetStableInstanceId() ?? 0})");
            // --- End Early Logging ---
            ...
            if (componentType == typeof(Transform))
            {
                ...
                // CodelyLogger.Log($"[GetComponentData] Manually serializing Transform (ID: {tr.GetStableInstanceId()})");
                return new Dictionary<string, object>
                {
                    { "typeName", componentType.FullName },
                    { "instanceID", tr.GetStableInstanceId() },
                    ...
                    { "parentInstanceID", tr.parent?.gameObject.GetStableInstanceId() ?? 0 },
                    { "rootInstanceID", tr.root?.gameObject.GetStableInstanceId() ?? 0 },
                    { "childCount", tr.childCount },
                    // Include standard Object/Component properties
                    { "name", tr.name }, 
                    { "tag", tr.tag }, 
                    { "gameObjectInstanceID", tr.gameObject?.GetStableInstanceId() ?? 0 }
                };
            }
            ...
            if (componentType == typeof(Camera))
            {
                ...
                var safeProperties = new Dictionary<string, Func<object>>
                {
                    ...
                    { "name", () => cam.name },
                    { "tag", () => cam.tag },
                    { "gameObject", () => new { name = cam.gameObject.name, instanceID = cam.gameObject.GetStableInstanceId() } }
                };
                ...
                {
                    { "typeName", componentType.FullName },
                    { "instanceID", cam.GetStableInstanceId() },
            ...
            {
                { "typeName", componentType.FullName },
                { "instanceID", c.GetStableInstanceId() }
            };
            ...
            // CodelyLogger.Log($"[GetComponentData] Starting property loop for {componentType.Name}...");
            ...
            foreach (var propInfo in cachedData.SerializableProperties)
            {
                ...
                if (propName == "rigidbody" || propName == "rigidbody2D" || propName == "camera" ||
                 ...
                 {
                     // CodelyLogger.Log($"[GetComponentData] Explicitly skipping generic property: {propName}"); // Optional log
                     skipProperty = true;
                 }
                ...
                {
                    // CodelyLogger.Log($"[GetComponentData] Explicitly skipping Camera property: {propName}");
                    skipProperty = true;
                }
                ...
                {
                    // CodelyLogger.Log($"[GetComponentData] Explicitly skipping Transform property: {propName}");
                    skipProperty = true;
                }
                // --- End Skip Transform Properties ---

                // --- Skip MeshFilter.mesh to avoid instantiating mesh in edit mode (use sharedMesh instead) ---
                if (!skipProperty &&
                ...
                // --- End Skip MeshFilter.mesh ---

                // --- Skip obsolete AudioSource properties (removed in Unity 5+) ---
                if (!skipProperty &&
                    componentType == typeof(AudioSource) &&
                    (propName == "minVolume" || propName == "maxVolume" || propName == "rolloffFactor"))
                {
                    skipProperty = true;
                }
                // --- End Skip AudioSource obsolete properties ---
                ...
                try
                {
                    ...
                    // CodelyLogger.Log($"[GetComponentData] Accessing: {componentType.Name}.{propName}");
                ...
                {
                     // CodelyLogger.LogWarning($"Could not read property {propName} on {componentType.Name}");
                }
            }

            // --- Add Logging Before Field Loop ---
            // CodelyLogger.Log($"[GetComponentData] Starting field loop for {componentType.Name}...");
            ...
            foreach (var fieldInfo in cachedData.SerializableFields)
            {
                 try
                {
                    ...
                    // CodelyLogger.Log($"[GetComponentData] Accessing Field: {componentType.Name}.{fieldInfo.Name}");
                ...
                {
                     // CodelyLogger.LogWarning($"Could not read field {fieldInfo.Name} on {componentType.Name}");
                }
            }
        ...
        private static void AddSerializableValue(Dictionary<string, object> dict, string name, Type type, object value)
        {
            ...
            catch (Exception e)
            {
                ...
                CodelyLogger.LogWarning($"[AddSerializableValue] Error processing value for '{name}' (Type: {type.FullName}): {e.Message}. Skipping.");
            }
        }
        ...
        private static object ConvertJTokenToPlainObject(JToken token)
        {
            ...
            switch (token.Type)
            {
                ...
                default:
                    ...
                    // CodelyLogger.LogWarning($"Unsupported JTokenType encountered: {token.Type}. Returning null.");
                    return null;
            }
        }
        ...
        private static readonly JsonSerializerSettings _outputSerializerSettings = new JsonSerializerSettings
        {
            Converters = new List<JsonConverter>
            {
                ...
                new RectConverter(),
                new BoundsConverter(),
                new Matrix4x4Converter(), // Prevents stack overflow from Matrix4x4.inverse/transpose recursion and ValidTRS asserts
                new TransformHandleConverter(), // Prevents AssertHandleIsValid throws when reflecting Unity 6 TransformHandle
        ...
        private static JToken CreateTokenFromValue(object value, Type type)
        {
            ...
            {
                CodelyLogger.LogWarning($"[GameObjectSerializer] Codely.Newtonsoft.Json Error serializing value of type {type.FullName}: {e.Message}. Skipping property/field.");
                return null; // Indicate serialization failure
            }
            catch (Exception e) // Catch other unexpected errors
            {
                CodelyLogger.LogWarning($"[GameObjectSerializer] Unexpected error serializing value of type {type.FullName}: {e}. Skipping property/field.");
                return null; // Indicate serialization failure
            }
        }
    }
