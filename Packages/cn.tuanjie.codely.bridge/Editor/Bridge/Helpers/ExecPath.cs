...
namespace UnityTcp.Editor.Helpers
{
    internal static class ExecPath
    {
            ...
            CancellationToken cancel)
        {
            ...
            try
            {
                ...
                using (var process = new Process { StartInfo = psi, EnableRaisingEvents = false })
                {
                    ...
                    if (!WaitForExitCancellable(process, timeoutMs, cancel))
                    {
        ...
        // Poll so a long-running process timeout can still be cancelled without
        // blocking the caller on a single WaitForExit(120000).
        private static bool WaitForExitCancellable(
            Process process, int timeoutMs, CancellationToken cancel)
        {
            ...
            if (timeoutMs <= 0)
            {
                while (!process.HasExited)
                {
                    ...
                    if (process.WaitForExit(sliceMs)) return true;
                }
            ...
            {
                if (cancel.IsCancellationRequested) return false;
                int wait = Math.Min(sliceMs, remaining);
                if (process.WaitForExit(wait)) return true;
                remaining -= wait;
            }
