...
namespace UnityTcp.Editor.Tools
{
    ...
    public static class ManageScript
    {
        ...
        private static readonly string[] ScriptActions =
        {
            "create", "read", "update", "delete",
            "apply_text_edits", "validate", "edit", "get_sha",
        };
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            if (!ActionRouter.TryResolve(@params, ScriptActions, out action, out var resolveError))
            {
                return resolveError;
            }
            ...
            return ActionRouter.Route(@params, BuildActionHandlers(
                fullPath, relativePath, name, contents, scriptType, namespaceName));
        }
        ...
        private static Dictionary<string, Func<JObject, object>> BuildActionHandlers(
            ...
            {
                { "create", _ => CreateScript(
                    fullPath, relativePath, name, contents, scriptType, namespaceName) },
                { "read", _ =>
                {
                    CodelyLogger.LogWarning("manage_script.read is deprecated; prefer resources/read. Serving read for backward compatibility.");
                    return ReadScript(fullPath, relativePath);
                } },
                { "update", _ =>
                {
                    CodelyLogger.LogWarning("manage_script.update is deprecated; prefer apply_text_edits. Serving update for backward compatibility.");
                    return UpdateScript(fullPath, relativePath, name, contents);
                } },
                { "delete", _ => DeleteScript(fullPath, relativePath) },
                { "apply_text_edits", p =>
                {
                    var textEdits = p["edits"] as JArray;
                    string precondition = p["precondition_sha256"]?.ToString();
                    string refreshOpt = p["options"]?["refresh"]?.ToString()?.ToLowerInvariant();
                    string validateOpt = p["options"]?["validate"]?.ToString()?.ToLowerInvariant();
                    return ApplyTextEdits(fullPath, relativePath, name, textEdits, precondition, refreshOpt, validateOpt);
                } },
                { "validate", p => ValidateScriptAction(fullPath, p) },
                { "edit", p =>
                {
                    CodelyLogger.LogWarning("manage_script.edit is deprecated; prefer apply_text_edits. Serving structured edit for backward compatibility.");
                    return EditScript(fullPath, relativePath, name, p["edits"] as JArray, p["options"] as JObject);
                } },
                { "get_sha", _ => GetShaAction(fullPath, relativePath) },
            };

        private static object ValidateScriptAction(string fullPath, JObject @params)
        {
        ...
        private static object GetShaAction(string fullPath, string relativePath)
        {
            try
            {
        ...
        )
        {
            ...
            else if (validationErrors != null && validationErrors.Length > 0)
            {
                ...
                CodelyLogger.LogWarning($"Script validation warnings for {name}:\n" + string.Join("\n", validationErrors));
            }
            ...
            string validateMode = null)
        {
            ...
            try
            {
                ...
                if (immediate)
                {
                    CodelyLogger.Log($"[ManageScript] ApplyTextEdits: immediate refresh for '{relativePath}'");
                    AssetDatabase.ImportAsset(
                        relativePath,
                        ImportAssetOptions.ForceSynchronousImport | ImportAssetOptions.ForceUpdate
                    );
#if UNITY_EDITOR
                    // A synchronous ImportAsset of a script can already start compilation (Bee).
                    // Skip the explicit request in that case to avoid re-entering / racing it.
                    if (!CompilationHelper.IsCompiling())
                    {
                        CompilationHelper.RequestScriptCompilationSafe();
                    }
#endif
                }
                else
                {
                    CodelyLogger.Log($"[ManageScript] ApplyTextEdits: debounced refresh scheduled for '{relativePath}'");
                    ManageScriptRefreshHelpers.ScheduleScriptRefresh(relativePath);
                }
            ...
            JObject options)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(validateOpt))
                    {
                        switch (validateOpt)
                        {
                            ...
                            default: /* keep existing level */ break;
                        }
                    }
                }
                ...
                if (immediate)
                {
                    CodelyLogger.Verbose($"[ManageScript] EditScript: immediate refresh for '{relativePath}'");
                    ManageScriptRefreshHelpers.ImportAndRequestCompile(relativePath);
                }
                else
                {
        ...
        private static bool ValidateClassSnippet(string snippet, string expectedName, out string err)
        {
            ...
            try
            {
                ...
                var classes = root.DescendantNodes().OfType<Codely.Microsoft.CodeAnalysis.CSharp.Syntax.ClassDeclarationSyntax>().ToList();
        ...
        private static bool TryComputeClassSpan(string source, string className, string ns, out int start, out int length, out string why)
        {
            ...
            try
            {
                ...
                var classes = root.DescendantNodes()
                    .OfType<Codely.Microsoft.CodeAnalysis.CSharp.Syntax.ClassDeclarationSyntax>()
                    .Where(c => c.Identifier.ValueText == className);

                if (!string.IsNullOrEmpty(ns))
                {
                    classes = classes.Where(c =>
                        (c.FirstAncestorOrSelf<Codely.Microsoft.CodeAnalysis.CSharp.Syntax.NamespaceDeclarationSyntax>()?.Name?.ToString() ?? "") == ns
                        || (c.FirstAncestorOrSelf<Codely.Microsoft.CodeAnalysis.CSharp.Syntax.FileScopedNamespaceDeclarationSyntax>()?.Name?.ToString() ?? "") == ns);
                }
        ...
        {
            string savedLevel = EditorPrefs.GetString("UnityTcp_ScriptValidationLevel", "standard");
            switch (savedLevel.ToLower())
            {
                case "basic": return ValidationLevel.Basic;
                case "standard": return ValidationLevel.Standard;
                case "comprehensive": return ValidationLevel.Comprehensive;
                case "strict": return ValidationLevel.Strict;
                default: return ValidationLevel.Standard;
            }
        }
        ...
        private static System.Collections.Generic.List<MetadataReference> GetCompilationReferences()
        {
            ...
            try
            {
                ...
                // Core .NET assemblies — use a HashSet to avoid duplicate references
                ...
                foreach (var t in coreTypes)
                {
                    ...
                    if (!string.IsNullOrEmpty(loc) && addedLocations.Add(loc))
                        references.Add(MetadataReference.CreateFromFile(loc));
                }

                // netstandard / System.Runtime / System.Core — required by Roslyn to resolve
                // base types like System.Object (CS0012) when doing full semantic compilation
                ...
                {
                    CodelyLogger.LogWarning($"Could not load UnityEngine assembly: {ex.Message}");
                }
                ...
                {
                    CodelyLogger.LogWarning($"Could not load UnityEditor assembly: {ex.Message}");
                }
                ...
                {
                    CodelyLogger.LogWarning($"Could not load Unity project assemblies: {ex.Message}");
                }
#endif
            ...
            {
                CodelyLogger.LogError($"Failed to get compilation references: {ex.Message}");
...
#if UNITY_EDITOR
            // The synchronous imports above can already have started compilation (Bee); only
            // request one when none is in flight so we don't re-enter and race it.
            if (!CompilationHelper.IsCompiling())
            {
                CompilationHelper.RequestScriptCompilationSafe();
            }
...
#if UNITY_EDITOR
        // A synchronous ImportAsset can already have started compilation (Bee); only request
        // one when none is in flight so we don't re-enter and race it.
        if (!CompilationHelper.IsCompiling())
        {
            CompilationHelper.RequestScriptCompilationSafe();
        }
